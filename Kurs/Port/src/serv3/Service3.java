...
public class Service3 {
    ...
    public void readFromJson() throws IOException {
        ...
        FileReader fileReader = new FileReader("Port/resources/Schedule.json");
    ...
    public void Unload(SumStat statistic) throws IOException, InterruptedException {
       ...
       /*
        if(arrayDry.size() != 0) {
            while(penaltyWaitingDry >= costCrane) {
                Cranes.setArrayT(arrayDry);
                cranesDry.incCount();
                cranesDry.setStat(stat);

                for (int i = 0; i < cranesDry.getCount(); i++) {
                    Thread t = new Thread(new Cranes());
                    threads.add(t);
                    t.start();
                }
                penaltyWaitingDry = Cranes.getPenalty();
            }
            queueDrySize = cranesDry.getQueueTSize();
            arrayDry = Cranes.getArrayT();
        }

        if(arrayLiquid.size() != 0) {
            while(penaltyWaitingLiquid >= costCrane) {
                Cranes.setArrayT(arrayLiquid);
                cranesLiquid.incCount();
                cranesLiquid.setStat(stat);
                ...
                penaltyWaitingLiquid = Cranes.getPenalty();
            }
            queueLiquidSize = cranesLiquid.getQueueTSize();
            arrayLiquid = Cranes.getArrayT();
        }

        if(arrayCont.size() != 0) {
            while(penaltyWaitingCont >= costCrane) {
                Cranes.setArrayT(arrayCont);
                cranesCont.incCount();
                cranesCont.setStat(stat);
                ...
                penaltyWaitingCont = Cranes.getPenalty();
            }
            queueContSize = cranesCont.getQueueTSize();
            arrayCont = Cranes.getArrayT();
        }
